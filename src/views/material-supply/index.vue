        ...
        <div class="map">
          <div class="map-wrapper">
            <map-core ref="map"
                      ...
                      :options="options"
                      @markClick="markClickHandle"></map-core>
...
import bar01 from "@/views/material-supply/subs/bar01";
import initBar01 from "@/views/material-supply/subs/bar01";
...
export default {
  ...
  methods: {
    ...
    // 基于乡镇统计
    async getTownData() {
      const res = await service("get", "/gep/stat/town/wzgj", {
        year: this.year,
    ...
    async getData(region, cb) {
      const res = await service("get", "/gep/profile/wzgj", {
        year: this.year,
        // parent: region ? region : this.region,
        // type: this.statisticType,
      });
      if (res.status === 200) {
        if (cb) {
          ...
          this.totalMaterialSupplyDataList = this.totalMaterialSupplyDataList.map(item => ({
            ...item,
            value: (res.data[item.key]).toFixed(2)
          }))
          this.$nextTick(() => {
            initBar01('barAdjust', this.totalMaterialSupplyDataList)

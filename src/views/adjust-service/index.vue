      ...
      <div>
        ...
        <Left :open="openLeft" @toggle="toggleLeftHandler" :showToggleBtn="false">
          ...
          <section class="page-container__section">
            <page-title>调节服务总量</page-title>
            <total-amount-service-regulation ref="totalAmountServiceRegulation" :total="totalAdjustServiceDataTotal"
            :dataList="totalAdjustServiceDataList"></total-amount-service-regulation>
          </section>
          <section class="page-container__section mt-5">
            <page-title>调节服务区域统计</page-title>
            <area-statistic-progress :dataList="dataTownList"></area-statistic-progress>
          </section>
        </Left>
        ...
        <div class="map">
          <div class="map-wrapper">
            <map-core ref="map" :map-click="true" :output-res="outputRes" @mapLayerClick="mapLayerClickHandle" :mapData="dataTownList" :type="searchParams.type" :options="options" @markClick="markClickHandle"></map-core>
...
import mapCore from "../home/subs/map/index.vue";
...
export default {
  ...
  data() {
    return {
      ...
      totalAdjustServiceDataTotal: 0,
      totalAdjustServiceDataList: [
        {
          name: '水源涵养',
          key: 'syHy',
          value: ''
  ...
  methods: {
    ...
    async getProCondWithUrl() {
      ...
      const res = await service("POST", "getProCondWithUrl", {
        year: this.year,
        projectName: this.current.properties.name,
        region: this.region,
        page: 1,
        perPage: 1,
        type: 'VEP_tj'
    ...
    // 基于乡镇统计
    async getTownData() {
      const res = await service("get", "/gep/stat/town/tjfw", {
        year: this.year,
    ...
    search() {
      this.dataTownList.forEach(item => {
        item.value = (this.searchParams.type ? item[this.searchParams.type] : item.total)
      })
      this.$nextTick(() => {
    ...
    async getData(region, cb) {
      const res = await service("get", "/gep/profile/tjfw", {
        year: this.year,
        parent: region ? region : this.region,
        type: this.statisticType,
      });
      if (res.status === 200) {
        if (cb) {
          ...
          if(this.year == 2023) {
              this.totalAdjustServiceDataTotal = 419.87
          } else {
              this.totalAdjustServiceDataTotal = (res.data.total).toFixed(2)
          }
          this.totalAdjustServiceDataList = this.totalAdjustServiceDataList.map(item => ({
            ...item,

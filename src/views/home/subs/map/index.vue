    ...
    if (this.mapClick) {
      _this.map.on('click', async (e) => {
        ...
        if (res.features && res.features.length > 0) {
          ...
          if(this.popupType === 'VEP_wh') {
            let id = ''
            if (feature.id) {
              const idArr = feature.id.split('.')
              id = (feature.properties.regionnum ? feature.properties.regionnum + '-' : '') + idArr[idArr.length - 1];
            } else {
              id = '-'
            }
            let popup = L.popup()
...
<div class="popup-item"><div class="popup-item__label">地块编号：</div><div class="popup-item__value">${id}</div></div>
...
<div class="popup-item"><div class="popup-item__label">文化服务价值：</div><div class="popup-item__value">${(feature.properties.wenhua_jz).toFixed(2)}元</div></div>
</div>`)
              .openOn(this.map);
          } else if (this.popupType === 'VEP_gj') {
            let id = ''
            if (feature.id) {
              const idArr = feature.id.split('.')
              id = (feature.properties.regionnum ? feature.properties.regionnum + '-' : '') + idArr[idArr.length - 1];
...
<div class="popup-item"><div class="popup-item__label">地块编号：</div><div class="popup-item__value">${id}</div></div>
<div class="popup-item"><div class="popup-item__label">地块生态类型：</div><div class="popup-item__value">${this.getDict('ecoDLBM', feature.properties.ecoDLBM)}</div></div>
<div class="popup-item"><div class="popup-item__label">物质供给价值：</div><div class="popup-item__value">${(feature.properties.GONGJI_jz).toFixed(2)}元</div></div>
</div>`).openOn(this.map);
          } else {
            let id = ''
            if (feature.id) {
            ...
            let popup = L.popup()
              ...
              .setContent(`<div class="leaflet-map-popup popup">
<div class="popup-item"><div class="popup-item__label">地块编号：</div><div class="popup-item__value">${id}</div></div>
<div class="popup-item"><div class="popup-item__label">地块面积：</div><div class="popup-item__value">${feature.properties.Shape_Area ? (feature.properties.Shape_Area).toFixed(2) : '-'}平方米</div></div>
<div class="popup-item"><div class="popup-item__label">地块生态类型：</div><div class="popup-item__value">${this.getDict('ecoDLBM', feature.properties.ecoDLBM)}</div></div>
<div class="popup-item"><div class="popup-item__label">调节服务价值：</div><div class="popup-item__value">${(feature.properties.tiaojie_jz || feature.properties.wenhua_jz).toFixed(2)}元</div></div>
</div>`)
              .openOn(this.map);
          }
